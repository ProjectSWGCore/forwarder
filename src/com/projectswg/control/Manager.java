...
public abstract class Manager extends Service {
	...
	public boolean initialize() {
		...
		synchronized (children) {
			for (Service child : children) {
				if (!child.initialize()) {
					Log.err(this, child.getClass().getSimpleName() + " failed to initialize!");
					success = false;
					break;
				}
			}
	...
	public boolean start() {
		...
		synchronized (children) {
			for (Service child : children) {
				if (!child.start()) {
					Log.err(this, child.getClass().getSimpleName() + " failed to start!");
					success = false;
					break;
				}
			}
	...
	public boolean stop() {
		...
		synchronized (children) {
			for (Service child : children) {
				...
				cSuccess = child.stop();
				if (!cSuccess) {
					Log.err(this, child.getClass().getSimpleName() + " failed to stop!");
					success = false;
				}
			}
		}
